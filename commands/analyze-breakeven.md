...
This command is a shortcut for the **position-manager** skill. Follow the full procedure described in `position-manager/SKILL.md` exactly — safety rules first, then parameter validation, data fetching, math, validation, and output.
...
Do not duplicate or reimplement the workflow here — always defer to `position-manager/SKILL.md` and its related files so the command and the skill never drift out of sync.
